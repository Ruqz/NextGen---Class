...
const FIELD_TYPE_OPTIONS: { value: FormFieldType; label: string; group: string }[] = [
  { value: 'text', label: 'Short Text', group: 'Text & Input' },
  { value: 'textarea', label: 'Long Text / Paragraph', group: 'Text & Input' },
  { value: 'email', label: 'Email Address', group: 'Text & Input' },
  { value: 'phone', label: 'Phone Number', group: 'Text & Input' },
  { value: 'number', label: 'Numeric Value', group: 'Text & Input' },
  { value: 'url', label: 'Website / URL', group: 'Text & Input' },
  { value: 'date', label: 'Date Selector', group: 'Dates & Choices' },
  { value: 'yes_no', label: 'Yes / No Toggle', group: 'Dates & Choices' },
  { value: 'single_choice', label: 'Single Choice (Radio)', group: 'Dates & Choices' },
  { value: 'multiple_choice', label: 'Multiple Choice (Checkboxes)', group: 'Dates & Choices' },
  ...
  { value: 'file_upload', label: 'File Upload Document', group: 'Attachments' },
...
export const FormFieldEditorModal: React.FC<FormFieldEditorModalProps> = ({
  ...
  const [activeTab, setActiveTab] = useState<'basic' | 'options' | 'file' | 'logic'>('basic');
  ...
  const [options, setOptions] = useState<string[]>(field?.options || ['Option 1', 'Option 2']);
  ...
  const isChoiceField = ['single_choice', 'multiple_choice', 'dropdown'].includes(fieldType);
        ...
        <div className="px-6 py-4 bg-slate-900 text-white flex items-center justify-between">
          <div>
            <h3 className="text-base font-bold flex items-center gap-2">
              ...
              {field ? 'Configure Question / Field' : 'Add New Application Question'}
            </h3>
            <p className="text-xs text-slate-400 mt-0.5">
              Customize field properties, validation rules, and dynamic display logic.
            </p>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="flex border-b border-slate-200 bg-slate-50 px-6 gap-2 text-xs font-semibold">
          ...
          {isFileField && (
            <button
              ...
              className={`py-3 px-3 border-b-2 cursor-pointer transition flex items-center gap-1.5 ${
                activeTab === 'file'
                  ? 'border-orange-600 text-orange-600'
                  : 'border-transparent text-slate-500 hover:text-slate-800'
              }`}
          ...
          >
            <GitBranch className="w-3.5 h-3.5" /> Conditional Rules {enableLogic && '•'}
          </button>
        </div>
        ...
        <div className="p-6 space-y-5 overflow-y-auto max-h-[60vh]">
          {activeTab === 'basic' && (
            <div className="space-y-4">
              ...
              <Input
                label="Question Label / Prompt"
                placeholder="e.g. What is your primary programming language?"
                value={label}
              ...
              <Input
                ...
                placeholder="e.g. Include years of experience or key frameworks used"
                value={description}
                onChange={(e) => setDescription(e.target.value)}
              />

              {!['file_upload', 'yes_no', 'checkbox', 'info_text'].includes(fieldType) && (
                <Input
              ...
              <div className="pt-2 flex flex-wrap items-center gap-6 border-t border-slate-100">
                <label className="flex items-center gap-2 text-xs font-semibold text-slate-800 cursor-pointer">
                  ...
                  />
                  Required Field (Applicant must answer)
                </label>
          ...
          {activeTab === 'options' && (
            <div className="space-y-4">
              <p className="text-xs text-slate-600">
                Configure the choices available to applicants for this question.
              </p>
