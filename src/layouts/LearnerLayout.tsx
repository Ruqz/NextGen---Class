...
export const LearnerLayout: React.FC<LearnerLayoutProps> = ({
  ...
  const { userProfile, enrolments, logout } = useAuth();
  const [mobileMenuOpen, setMobileMenuOpen] = useState(false);

  const activeEnrolment = enrolments && enrolments.length > 0 ? enrolments[0] : null;

  const navItems = [
    { label: 'DASHBOARD', path: '/learner/dashboard', icon: <LayoutDashboard className="w-4 h-4" /> },
    { label: 'MY CLASSES', path: '/learner/classes', icon: <Calendar className="w-4 h-4" /> },
    { label: 'RESOURCES', path: '/learner/resources', icon: <BookOpen className="w-4 h-4" /> },
    { label: 'ASSIGNMENTS & CAPSTONE', path: '/learner/assignments', icon: <ClipboardList className="w-4 h-4" /> },
    { label: 'ATTENDANCE', path: '/learner/attendance', icon: <UserCheck className="w-4 h-4" /> },
    { label: 'PROGRESS', path: '/learner/progress', icon: <BarChart3 className="w-4 h-4" /> },
    { label: 'FEEDBACK', path: '/learner/feedback', icon: <HelpCircle className="w-4 h-4" /> },
    { label: 'CERTIFICATE', path: '/learner/certificate', icon: <Award className="w-4 h-4" /> },
    { label: 'PROFILE', path: '/learner/profile', icon: <User className="w-4 h-4" /> },
  ];
      ...
      <header className="sticky top-0 z-40 bg-white border-b border-slate-200 shadow-2xs">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16">
            {/* Logo */}
            <div className="flex items-center gap-3 cursor-pointer" onClick={() => handleNav('/learner/dashboard')}>
              ...
              <div>
                <div className="flex items-center gap-2">
                  <span className="font-bold text-lg text-slate-900 tracking-tight leading-none">
                    NextGen <span className="text-emerald-600">Class</span>
                  </span>
                  <Badge variant="success" className="text-[9px] uppercase font-black px-1.5 py-0.5">
                    LEARNER PORTAL
                  </Badge>
                </div>
                <p className="text-[11px] text-slate-500 font-medium hidden sm:block">
                  {activeEnrolment?.programmeName || 'Gen AI & Tech Skills Hub'}
                </p>
              </div>
            </div>
            ...
            <div className="flex items-center gap-3">
              <div className="hidden sm:flex items-center gap-2.5 pl-2 border-l border-slate-200">
                <div className="w-8 h-8 rounded-full bg-emerald-100 text-emerald-700 flex items-center justify-center font-bold text-xs">
                  {userProfile?.displayName ? userProfile.displayName.charAt(0).toUpperCase() : 'L'}
                </div>
                <div className="text-left hidden md:block">
                  ...
                  <p className="text-[11px] text-slate-500 truncate max-w-[130px]">
                    {activeEnrolment?.cohortName || userProfile?.email}
                  </p>
                </div>
              </div>
        ...
        <aside className="hidden md:flex w-64 bg-slate-900 text-slate-300 min-h-[calc(100vh-4rem)] flex-col justify-between shrink-0 border-r border-slate-800">
          <div className="p-4 space-y-4">
            ...
            <nav className="space-y-1">
              {navItems.map((item) => {
                const isActive = currentPath === item.path || (item.path !== '/learner/dashboard' && currentPath.startsWith(item.path));
                return (
                  <button
                    key={item.path}
          ...
          <div className="p-4 border-t border-slate-800 bg-slate-900/90 text-xs text-slate-400">
            <p className="font-semibold text-slate-200">Enrolled Student</p>
            <p className="text-[11px] mt-1 text-emerald-400 font-mono">
              {activeEnrolment?.enrolmentCode || 'ENR-ACTIVE'}
            </p>
          </div>
        </aside>
          ...
          <div className="fixed inset-0 z-50 flex md:hidden">
            ...
            <div className="relative flex-1 max-w-xs w-full bg-slate-900 text-slate-300 z-10 flex flex-col h-full shadow-2xl p-4 space-y-4">
              <div className="flex items-center justify-between pb-3 border-b border-slate-800">
                <span className="text-xs font-bold uppercase text-emerald-400">Learner Portal</span>
                ...
                {navItems.map((item) => (
                  <button
                    ...
                    className={`w-full flex items-center justify-between px-3 py-2.5 text-xs font-medium rounded-xl transition-all ${
                      currentPath === item.path ? 'bg-emerald-600 text-white' : 'text-slate-300 hover:bg-slate-800'
                    }`}
                  >
                    <div className="flex items-center gap-2.5">
                      {item.icon}
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 pb-6 mb-6 border-b border-slate-200">
            <div>
              <div className="flex items-center gap-2 mb-1">
                <Badge variant="success" className="text-[10px]">ENROLLED LEARNER</Badge>
                <span className="text-xs text-slate-500 font-medium">Cohort Learning Workspace</span>
              </div>
